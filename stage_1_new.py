...
WITH_GROUPING = True
SEED_list=[i for i in range(0,11)]
...
for turn in range(len(SEED_list)):
    ...
    train_0=train_0.sample(n=2500,replace=False,random_state=SEED_list[turn])
    ...
    for i in range(1,6):
        train=train.append(temp_train.loc[temp_train['Churn Category'] == i])

    train['Group Label'] = np.array(list(range(len(train))))


    for i in range(1,6):
        train=train.append(temp_train.loc[temp_train['Churn Category'] == i].sample(n=int(N-num_list[i]),replace=True,random_state=SEED_list[turn]))
    ...
    # save_model
    stage_2_model.save_model(f'./stage_1_en/stage_1_model_{turn}.json')
